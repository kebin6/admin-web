...
SUB_FOLDER=order
API_FILE=../wolflamp-api/desc/${SUB_FOLDER}.api
...
.PHONY: gen-web
gen-web:
	goctls frontend vben --api_file=${API_FILE} --output=./ --folder_name=${FOLDER_NAME} --prefix=${PREFIX} --sub_folder=${SUB_FOLDER} --overwrite=${OVERWRITE} --form_type=${FORM_TYPE}
